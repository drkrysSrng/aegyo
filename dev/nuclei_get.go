...
func nuclei_get(domain string) {
	...
	go func() {
		defer sg.Done()
		err = ne.ExecuteNucleiWithOpts([]string{domain},
	...
	// scan 2 = run templates with oast tags on domain
	sg.Add()
	go func() {
		defer sg.Done()
		err = ne.ExecuteNucleiWithOpts([]string{domain}, nuclei.WithTemplateFilters(nuclei.TemplateFilters{Tags: []string{"oast"}}))
		if err != nil {
			panic(err)
		}
	}()
